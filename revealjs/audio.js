...
window.RevealAudioSlideshow = window.RevealAudioSlideshow || {
	...
	stop: function(deck) {
		stopAudioSlideshow(deck);
	},
	updateNotes: function(deck) {
		updateNotes(deck);
	}
};
var currentPlaylist = null;

const stopAudioSlideshow = function(Reveal) {
	if ( currentPlaylist ) { 
...
const updateNotes = function(deck) {
	var slide = deck.getCurrentSlide();
	let notes = slide.getAttribute( 'data-journalentry' );
	$('#slideshow_slideshow_notes_content .content').html("");
	
	if (notes > "") {
		const journal = game.journal;
		if (journal != null) {
			
			const note = journal.getName(notes);
			if ( note != null ) {
				
				const name = note.data.name;
				const content = note.data.content;
				$('#slideshow_slideshow_notes_content .content').html(name + content);
...
console.debug( "slidechanged ");
	...
	Reveal.addEventListener( 'stopped', function( event ) {
		if ( timer ) { clearTimeout( timer ); timer = null; }
		if ( currentPlaylist ) { 
			currentPlaylist.stopAll(); 
			currentPlaylist = null; 
	...
	function selectAudio( previousAudio ) {
		if (game.user.isGM) {
			...
			let audio = slide.getAttribute( 'data-audio-src' );
			if (audio > "") {
				const playlists = game.playlists;
				if (playlists != null) {
					const playlist = playlists.getName(audio);
